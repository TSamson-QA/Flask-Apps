...
app = Flask(__name__)
app.config['SECRET_KEY']='bewcubbcc' #Configure a secret key for CSRF protection.
...
class myForm(FlaskForm):
    username = StringField('Username', validators=[
        ...
        UserCheck(message="username not available",banned = ['root','admin','sys']),
        NoneOf(values=['#','$','£','&'], message="please use valid characters"),
        Length(min=2,max=15)
        ])
